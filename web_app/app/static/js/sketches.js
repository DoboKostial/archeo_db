...
(function () {
  ...
  const grouped = new Map(); // name -> [values]
  inputs.forEach(inp => {
    const name = inp.dataset.target;
    const id = String(inp.value);
    if (!grouped.has(name)) grouped.set(name, []);
    grouped.get(name).push(id);
  });

  grouped.forEach((ids, name) => {
    const ss = root.querySelector(`.search-select[data-hidden-name="${name}"]`);
    if (!ss || !ss._searchSelect) return;

    ss._searchSelect.clear();

    const mode = ss.dataset.mode || "multi";
    if (mode === "single") {
      const v = ids[0];
      ss._searchSelect.setSingle(v, v);
    } else {
      const values = ids.map(v => {
        const text =
          (name === "ref_sj") ? `SJ ${v}` :
          (name === "ref_section") ? `Section ${v}` :
          String(v);
        return { id: v, text };
      });
      ss._searchSelect.setMulti(values);
    }
  });
}
...
prefillSearchSelects(document);
  ...
  const checkboxSelector = ".sketch-check, .photo-check, .item-check";
  const hiddenInputName = "sketch_ids"; // adjust in template/backend if needed
  ...
  const editId = document.getElementById("editIdSketch") || document.getElementById("editId");
  const deleteId = document.getElementById("deleteIdSketch") || document.getElementById("deleteId");
  ...
  async function openEdit(idSketch) {
    const url = `/sketches/api/detail/${encodeURIComponent(idSketch)}`;
    const resp = await fetch(url, { credentials: "same-origin" });
    if (!resp.ok) return;
    const data = await resp.json();

    if (editId) editId.textContent = data.id_sketch;
    if (editForm) editForm.action = `/sketches/edit/${encodeURIComponent(data.id_sketch)}`;

    // basic fields
    setVal(editForm, 'select[name="sketch_typ"]', data.sketch_typ);
    setVal(editForm, 'input[name="datum"]', data.datum || "");
    setVal(editForm, 'input[name="notes"]', data.notes || "");

    // author (single search-select)
    ssSetSingle(editForm, "author", data.author, data.author);
    ...
    ssSetMulti(editForm, "ref_find", links.find_ids || [], "Find");
    ssSetMulti(editForm, "ref_sample", links.sample_ids || [], "Sample");

    if (bsEdit) bsEdit.show();
  }

  function openDelete(idSketch) {
    if (deleteId) deleteId.textContent = idSketch;
    if (deleteForm) deleteForm.action = `/sketches/delete/${encodeURIComponent(idSketch)}`;
